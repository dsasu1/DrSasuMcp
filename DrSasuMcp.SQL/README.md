...
Once you have your connection string configured, add the SQL tool to your MCP client:
...
```json
{
  "mcpServers": {
    "drsasumcp-sql": {
      "command": "C:\\Projects\\personal\\DrSasuMcp\\DrSasuMcp.SQL\\publish\\DrSasuMcp.SQL.exe",
      "env": {
        "SQL_CONNECTION_STRING": "Server=.;Database=YourDatabase;Trusted_Connection=True;TrustServerCertificate=True"
      }
    }
...
```json
{
  "mcpServers": {
    "drsasumcp-sql": {
      "command": "/path/to/DrSasuMcp/DrSasuMcp.SQL/publish/DrSasuMcp.SQL",
      "env": {
